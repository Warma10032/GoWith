...
| `test-agent` | 单元/集成/E2E | `tests/**`、`apps/ai-worker/tests/**`、`packages/shared/src/*.test.ts` | E2E 用本地 Chrome 通道，不下载 Playwright Chromium；TS 单测用 vitest |
...
3. schema-agent：写 migration（追加 005+ 或修改现有文件？需 review）；同步 packages/db/src/schema.ts（**当前 drift，见 §6**）。
...
2. **AI JSON gate**：所有 AI 测试用例的 `ai_runs.status` 必须为 `success` 且 `validation_status = valid`。当前唯一用例是 `apps/ai-worker/tests/test_health.py`。
3. **POI gate**：单元测试覆盖 `auto_matched` / `need_review` / `low_confidence` / `no_candidate` 四态 + 强制审核场景（M1 加）。
4. **Privacy gate**：禁止在 diff、log、test fixture 中出现明文 Cookie / 手机号 / 邮箱。
5. **Coord gate**：所有 `pois` / `shops` 新增 row 必须有 `coord_type`（DB CHECK 强制）。
6. **Evidence gate**：前台展示路径（首页 / 地图 / 详情）有专门的 Playwright 用例校验"AI 总结，仅供参考"提示 + 证据入口存在。`tests/e2e/smoke.spec.ts` 已在首页断言。
7. **Test coverage gate**：核心模块 ≥ 80%（详见 `docs/MVP-bilibili-shop-map.md` §12 指标体系）；vitest 在 `packages/shared` 已起步。
8. **Doc gate**：API 变更 / 状态机变更 / 新增枚举必须同步 `docs/MVP-*.md` + `docs/openapi.yaml`。
