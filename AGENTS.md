...
- **E2E 门槛**：保留 `tests/e2e/smoke.spec.ts` 一条 Playwright 即可覆盖首页 + 地图页 + 后台登录可达；新增 E2E 用例需人在 PR 描述里写理由。
...
4. test-agent：判断要不要写单测 —— 仅当功能属于 §2.8 列出的"重计算 / 重副作用 / 易回退"边界才写；其余跳过。
5. 对应领域 agent：写最小实现让代码通过 typecheck + 现有 smoke；如第 4 步写了单测则同时让测试变绿。
6. compliance-agent：跑合规 checklist（B站 / 坐标 / AI 声明 / 隐私）。
7. docs-agent：把字段、状态机、API 更新同步进 docs/。
8. 提交 PR；CI 跑 typecheck + lint + 现有测试；review-queue-agent 监控审核队列是否合理。
```
...
> **覆盖率和测试范围**：见 §2.8，本节不重复强制阈值。`pnpm test` 通过 = smoke 全绿（当前 ≈ 3 个测试 + 1 E2E），即可合入。
