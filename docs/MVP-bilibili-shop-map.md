...
- 状态管理：M0 用 `fetch` + RSC + fallback mock（`apps/web/src/lib/api.ts` 的 `fallbackShops`）；M4 引入 TanStack Query + Zustand。
...
- 数据库：PostgreSQL 16 + PostGIS（落地于 `db/migrations/00{1..4}_*.sql`）。
- 任务审计：DB `jobs` 表 + BullMQ 双写（`apps/api/src/services/queue.ts` 的 `enqueuePipelineJob`）。
...
| 里程碑 | 状态 | 关键交付 |
| --- | --- | --- |
| M0 脚手架 + Mock 闭环 | ✅ 完成 | monorepo、4 migration、API/Worker/AI-Web/Web/Admin 全部可用，全 mock |
