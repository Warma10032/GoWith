...
版本：v0.2  
日期：2026-06-28  
状态：M0/M1/M2 已完成，M3/M4/M5 部分完成；仓库已 Hard cut 到真实集成模式  
...
- 种子博主首次全量同步。
- 后续每日增量同步：先扫描该博主所有可访问视频分页，只补充未入库视频；已入库且非 `metadata_failed` 的视频不刷新、不重置状态、不自动重跑 AI/ASR。
- 若单个视频详情 / 字幕 / 评论 enrichment 失败，保留视频列表元数据并置为 `metadata_failed`；下一次同步只重跑这些失败视频和新发现视频，不重跑全量。
- B站返回限流 / 风控 / 网络类错误时，worker 记录错误分类并通过全局 B站请求节流进入 cooldown，降低后续请求并发压力。
- Worker 内置 `daily_creator_video_sync` 定时任务每天为所有 `active` 博主分发一次 `creator_video_sync`，用于自动补充最新视频；后台“定时任务”页可查看和手动触发。
...
- LLM：**MiniMax**（OpenAI-compatible SDK），`MINIMAX_MODEL=MiniMax-M3`，简单阶段可走 `MiniMax-M2.7`；`apps/ai-worker/app/main.py` 全部 5 阶段直接调 MiniMax，无 mock 兜底。缺失 `MINIMAX_API_KEY` 抛 `500 minimax_api_key_missing`。
- ASR：**Groq Whisper**，默认 `whisper-large-v3-turbo`（重识别时切 `whisper-large-v3`）；`/asr/transcribe` 接真实 Groq；缺失 `GROQ_API_KEY` 抛 `500 groq_api_key_missing`。长视频由 `_prepare_audio_chunks` 用 ffmpeg 切 16KHz mono FLAC 并按 `GROQ_ASR_MAX_MB=25` 自动再切片；应用层 `ASR_MAX_UPLOAD_MB=26` 流式截断兜底（P1-5）。
- AI Worker：**FastAPI + Pydantic v2 + uv**，落地于 `apps/ai-worker/`；mypy strict + ruff；内部 shared secret 鉴权（P0-4）。
...
- 结果缓存：按 `(stage, input_hash)` 重放；`ai_runs` 持久化原始输出 + `provider='minimax'` + `model_version`（`MiniMax-M3` / `MiniMax-M2.7`）。
...
### M1：数据采集跑通 ✅ 已完成（2026-06-28）

- 种子 UID 入库（admin 入口可用）。
- 视频元信息同步：`apps/worker/src/adapters/bilibili.ts` 接真实 HTTP，WBI 签名 + Cookie 池 + 429 自适应限流 + 全局请求节流。
- 增量同步：每个失败视频置 `metadata_failed`，下次同步只重跑失败 + 新发现视频，不重跑全量。
- 字幕获取：Groq Whisper `whisper-large-v3-turbo`，长视频 ffmpeg 切 16KHz mono FLAC 后按 25MB 再切片。
- 评论样本获取：热评（mode=3）+ 最新评论（mode=2）+ 关键词评论混合抽样，评论用户信息保存 `user_hash`。
- B 站 Cookie 健康检查 + 风控分类 + 过期账号定时清理。

### M2：AI 视频理解跑通 ✅ 已完成（2026-06-28）

- `apps/ai-worker/app/main.py` 接 MiniMax LLM（OpenAI-compatible SDK，`MiniMax-M3` 主、`MiniMax-M2.7` 简）。
- 让 AI 走 `prompt_version` + `input_hash` 重放，`ai_runs` 落 `provider='minimax'` + `model_version`。
- 5 阶段 JSON Schema 双层（Zod + Pydantic）校验已就绪。
- 错误分类 → `ai_runs.status ∈ {success, failed, invalid_json, schema_error}` + 触发对应 `review_tasks`。

### M3：POI 与后台跑通 🟡 部分（2026-06-28）

- 高德真实 POI 搜索：`apps/worker/src/adapters/poi.ts` 调 `v5/place/text`，写原始响应到 `raw_ingest_payloads`。
- 候选重排：dice 相似度 + 6 信号加权（name 0.46 / city 0.16 / district 0.10 / business_area 0.08 / address 0.12 / category 0.08）。
- 后台审核流程（POI 选择 / 店名修正 / 提交复审）。
- `POST /api/admin/shops/:id/publish` 事务 + `published_shop_snapshots` 版本化。
- `POST /api/admin/shops/merge` 仍为占位符。
- 发布状态流转 + `review_events` 审计日志（**待 `packages/db/src/schema.ts` 字段名修复后启用**）。
- 博主、视频、店铺支持带审计的软删除、回收站与恢复（migration 007）；删除博主时同批次隐藏其视频。
- 后台人工修订与 B站来源字段分离（`overrides` 表 + migration 007）。
- 已发布店铺必须先下架为 `hidden`，再修改、提交复审并重新发布。
- 店铺评论分析支持按维度编辑或从当前展示结果移除，历史 AI JSON 与证据保持不可变。
...
- `apps/worker/src/adapters/bilibili.ts` — B站真实 HTTP（WBI 签名 + Cookie 池 + 节流 + 风控 + 错误分类）
- `apps/worker/src/adapters/ai.ts` — 通过 HTTP 调 AI worker（带 shared secret）
- `apps/worker/src/adapters/poi.ts` — 高德 `v5/place/text` 真实搜索
- `apps/ai-worker/app/main.py` — MiniMax + Groq Whisper
...
生产安全加固（已 100% 落地，按 `docs/security-public-deployment-report.md`）：

- P0-1 生产 secret 强校验 / P0-2 登录限速 + 失败锁定 / P0-3 CSRF / P0-4 AI Worker 内部鉴权
- P0-5 Postgres + Redis + AI Worker 内网化 / P0-6 图片下载 SSRF 防护
- P1-1 生产错误脱敏 / P1-2 Swagger UI 仅 dev / P1-3 SSE CORS / P1-4 埋点限速 / P1-5 ASR 上传兜底
- P2-1 密钥轮换 envelope / P2-2 web 安全响应头 / P2-3 poi URL 脱敏 / P2-4 第三方 key 日志脱敏
