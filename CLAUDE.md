...
- DB 5 个 migration 已写好且与代码同步（`db/migrations/`）
...
| 数据库           | PostgreSQL 16 + PostGIS、pgcrypto、pg_trgm；schema 由 5 个 migration 落地                                                           | `db/migrations/`                                      |
...
迁移按五个 migration 推进：①扩展 + 基础表 → ②AI 与候选店铺表 → ③POI/shops/review 表 → ④推荐日志表 → ⑤高德商业字段与外部平台链接。注意 003 里 `shop_candidates.selected_poi_id` / `shop_candidates.merged_shop_id` / `evidence.shop_id` 的 FK 必须在 shops / pois 建好之后用 `ALTER TABLE ... ADD CONSTRAINT` 添加。
...
- Web: http://localhost:3170（容器内 next dev 仍跑 3000，宿主映射 3170 避开 Windows Hyper-V 保留段 TCP 2970-3069）
- API: http://localhost:4000（Swagger UI: `/docs`）
- AI Worker: http://localhost:8000
- Postgres: localhost:5540（避开同一保留段；.env `DATABASE_URL` 同步指向 5540）
