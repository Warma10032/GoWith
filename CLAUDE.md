...
- DB 6 个 migration 已写好且与代码同步（`db/migrations/`）
...
| 数据库           | PostgreSQL 16 + PostGIS、pgcrypto、pg_trgm；schema 由 6 个 migration 落地                                                           | `db/migrations/`                                      |
...
迁移按六个 migration 推进：①扩展 + 基础表 → ②AI 与候选店铺表 → ③POI/shops/review 表 → ④推荐日志表 → ⑤高德商业字段与外部平台链接 → ⑥移除 AI 人均字段。注意 003 里 `shop_candidates.selected_poi_id` / `shop_candidates.merged_shop_id` / `evidence.shop_id` 的 FK 必须在 shops / pois 建好之后用 `ALTER TABLE ... ADD CONSTRAINT` 添加。
...
- API: http://localhost:14000（Swagger UI: `/docs`）
